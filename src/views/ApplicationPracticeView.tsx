...
export const ApplicationPracticeView: React.FC<ApplicationPracticeViewProps> = ({ 
  ...
  const [scenarioPrompt, setScenarioPrompt] = useState(
    ...
    `You are designing a high-throughput mobile airtime/data top-up system for 500,000 Nigerian students during flash promo periods. When 10,000 concurrent students tap 'Purchase' simultaneously, the database thread pool collapses. How do you apply "${selectedConcept?.title || 'Concurrency'}" principles to ensure fairness, prevent race conditions, and keep API latency under 200ms?`
  );
  ...
  if (!selectedConcept) {
    return (
      <div className="max-w-4xl mx-auto p-8 text-center space-y-4">
        <h2 className="text-xl font-bold text-[#071A3A]">No Concepts Available for Application</h2>
        <button
          onClick={() => onNavigate('/app/capture/new')}
          className="px-5 py-2.5 rounded-xl bg-[#124BCE] text-white text-xs font-bold"
        >
          Capture Concept
        </button>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 text-amber-800 text-xs font-bold uppercase tracking-wider">
              ...
              <span>Operational Transfer Challenge</span>
            </div>
            <VisualCueTooltip
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-3 py-1.5 rounded-xl border border-gray-300 text-xs font-semibold bg-white text-[#071A3A] focus:ring-2 focus:ring-[#124BCE]"
          >
            {concepts.map(c => (
      ...
      {/* Main Challenge Card */}
      <div className="p-6 rounded-3xl bg-white border border-amber-200/80 shadow-xs space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-amber-800 uppercase">{selectedConcept.domain}</span>
              <span className="text-gray-300">•</span>
              <span className="text-xs text-gray-500">Far Transfer Scenario</span>
            </div>
            <h2 className="text-xl font-bold font-heading text-[#071A3A] mt-1">
        ...
        {/* Scenario Prompt Box */}
        <div className="p-5 rounded-2xl bg-gradient-to-br from-amber-50/70 to-orange-50/40 border border-amber-200 space-y-3">
          ...
          <p className="text-xs sm:text-sm text-gray-800 leading-relaxed whitespace-pre-line font-medium">
            {scenarioPrompt}
          </p>
        </div>
        ...
        {!isSubmitted ? (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-gray-400">
                  Must explain WHY your chosen approach satisfies the constraints
                </span>
              </div>
              <textarea
                value={userSolution}
                ...
                rows={6}
                required
                className="w-full p-4 rounded-2xl border border-gray-300 text-xs sm:text-sm text-[#071A3A] focus:outline-none focus:ring-2 focus:ring-[#124BCE] focus:border-transparent placeholder:text-gray-400 resize-y"
              />
            </div>
          ...
          <div className="space-y-6 pt-4 border-t border-gray-100 animate-in fade-in duration-200">
            ...
            <div className="p-4 rounded-2xl bg-[#F7FAFF] border border-[#1769FF]/20 flex items-center justify-between">
              <div>
                <div className="text-xs font-semibold text-[#124BCE]">State Advanced</div>
            ...
            <div className="flex justify-between items-center pt-2">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-[#124BCE] hover:bg-[#1769FF] text-white text-xs font-bold transition-colors cursor-pointer"
              >
